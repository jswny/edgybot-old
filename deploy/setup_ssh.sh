...
DEPLOY_KEY_VARIABLE=$(echo "${DEPLOY_KEY_ENV_VAR_PREFIX}_key")
DEPLOY_IV_VARIABLE=$(echo "${DEPLOY_KEY_ENV_VAR_PREFIX}_iv")
...
{ ssh-keyscan -t "$TRAVIS_SSH_KEY_TYPES" -H "$DEPLOY_SERVER" >> "${TRAVIS_HOME}"/.ssh/known_hosts; } 2>&1
